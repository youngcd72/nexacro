...
if (nexacro.Component) {
	...
	_pComponent._getLinkedLabel = function (label) {
		...
		if (label) {
			...
			for (var i = 0; i < len; i++) {
				var str = arr[i];
				var _arr = str.toString().replace(/\]/g, "]" + _rs_).split(_rs_);
				var _len = _arr.length;

				for (var j = 0; j < _len; j++) {
					var _str = _arr[j];
					var start = _str.search("@");
					var end = _str.search("]");

					start = (start < 0 || start != 1) ? 0 : start + 1;
					end = (end < 0) ? _str.length : end;

					var linkedId = _str.substr(start, end - start);
