 ...
 */
pForm.gfnFileUpload = function(objFileUpload, sUrl, sPath, paramDataSet)
{	
	...
	//파일 업로드 시킬 폴더 위치 지정
	this.gfnIsNull(sPath) ? sPath = "?PATH=upload" : sPath = "?PATH=" + sPath;
	
	if(this.gfnIsNull(paramDataSet) ) {
		paramValue = "";
	}
	else {
		paramValue = "&inputDatasets=";//+paramDataSet.saveSSV(); ;
	}
	
	trace("--> sFileUrl :"+sFileUrl );
	trace("--> sPath :"+ sPath);
	trace("--> paramValue :"+ paramValue);
	var bSucc = objFileUpload.upload(sFileUrl + sPath + paramValue);
	trace("bSucc >> " + bSucc);
};
