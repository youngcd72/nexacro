...
if (!nexacro.Grid) {
	...
	_pGridCell._common_fire_lbuttonup = function (touchinfos, changedtouchinfos, button, altKey, ctrlKey, shiftKey, screenX, screenY, canvasX, canvasY, clientX, clientY, obj, from_refer_comp, from_elem) {
		...
		if (changedtouchinfos) {
			...
			parent = grid.parent;
			retn = grid.on_fire_user_onlbuttonup(button, altKey, ctrlKey, shiftKey, screenX, screenY, canvasX, canvasY, clientX, clientY, orgcell, orgcell, from_elem, true);
				...
				if ((datarow == grid._selectinfo.curdsrow) && (cell._cellidx == grid._selectinfo.curcell)) {
					if (grid._showEditing) {
						...
						if (upelem) {
							...
							if (check) {
								cell.on_fire_onclick(button, altKey, ctrlKey, shiftKey, screenX, screenY, org_canvasX, org_canvasY, org_clientX, org_clientY, obj, from_refer_comp, "control", true);
							}
						}
					}
					else {
	...
	_pOverlayControl.setControlElemPosition = function (left, top, width, height, fixed_area_scroll) {
	...
	_pGridRow.on_fire_onclick = function (button, alt_key, ctrl_key, shift_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp) {
		...
		if (this._band) {
			this._band.on_fire_onclick(button, alt_key, ctrl_key, shift_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp);
		}
	};
	...
	_pGridBand.on_fire_onclick = function (button, alt_key, ctrl_key, shift_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp) {
		...
		if (grid.onnodataareaclick && grid.onnodataareaclick._has_handlers) {
			return grid.on_fire_onnodataareaclick(button, alt_key, ctrl_key, shift_key, screenX, screenY, canvasX, canvasY, clientX, clientY, grid, this);
		}
		return false;
	};
	...
	_pGrid.on_fire_sys_ondragmove = function (src_comp, src_refer_comp, dragdata, userdata, datatype, filelist, button, alt_key, ctrl_key, shift_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp, xdeltavalue, ydeltavalue) {
		if (this._selectscrollmode == "select") {
			...
			return nexacro.Component.prototype.on_fire_sys_ondragmove.call(this, src_comp, src_refer_comp, dragdata, userdata, datatype, filelist, button, alt_key, ctrl_key, shift_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp, xdeltavalue, ydeltavalue);
		}
	};
	...
	_pGrid._on_bubble_drag = function (elem, button, alt_key, ctrl_key, shift_key, canvasX, canvasY, screenX, screenY, event_bubbles, fire_comp, refer_comp) {
		if (!this._is_alive) {
			return;
		}
		var clientXY, canvas;
			...
			if (!event_bubbles || event_bubbles[0] !== true || is_parent_bubble) {
				if (pThis && (!pThis.ondrag || (pThis.ondrag && !pThis.ondrag.stoppropagation)) && !this._window && this.parent && !this.parent._is_application) {
					...
					if (is_subcontrol_bubble) {
						ret = this.parent._on_bubble_drag(elem, button, alt_key, ctrl_key, shift_key, canvasX, canvasY, screenX, screenY, event_bubbles, this, refer_comp);
					}
					else {
						ret = this.parent._on_bubble_drag(elem, button, alt_key, ctrl_key, shift_key, canvasX, canvasY, screenX, screenY, false, this, refer_comp);
					}
					bubblefire = true;
				}
			}
			...
			if ((!event_bubbles || event_bubbles[0] !== true || (event_bubbles[0] == true && this._noFireDragFlag == true)) && this.parent && !this.parent._is_application) {
				...
				if (!event_bubbles || event_bubbles[0] !== true || is_parent_bubble) {
					...
					if (pThis && (!pThis.ondrag || (pThis.ondrag && !pThis.ondrag.stoppropagation)) && !this._window && this.parent && !this.parent._is_application) {
						...
						ret = this.parent._on_bubble_drag(elem, button, alt_key, ctrl_key, shift_key, canvasX, canvasY, screenX, screenY, false, fire_comp, refer_comp);
						bubblefire = true;
					}
				}
			}
					...
					if (this._setdataobj && this._setdataobj.succ == false) {
						...
						if (this.autoenter == "select") {
							...
							if (cellobj) {
								...
								if (cellobj._rowidx != this.currentrow || cellobj._cellidx != this.currentcell) {
									bShowEditor = true;
								}
								else {
	...
	_pGrid._showEditor = function (focus_cellobj) {
		...
		var _win = mainframe._getWindow();
		var focuslist = _win.getCurrentFocusPaths();
		var len = focuslist.length;
		var last_focused_comp;

		if (len > 0) {
			last_focused_comp = focuslist[len - 1];
			if (last_focused_comp && last_focused_comp._is_alive) {
				activeframe = last_focused_comp._getOwnerFrame();
			}
		}
		if (activeframe && activeframe._is_alive) {
			if (owner_frame != activeframe) {
				is_activate = false;
			}
		}
	...
	_pGrid._setOverlayControlProperty = function (control, left, top, width, height, style_cells, target_datarow, target_cellinfo, is_create_ctrl, change_size) {
		...
		control.setControlElemPosition(left, top, width, height, change_size);
