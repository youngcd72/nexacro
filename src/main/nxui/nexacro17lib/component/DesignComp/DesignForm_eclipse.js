...
if (nexacro.DesignForm) {
	...
	_pSnapManager.add_dot_info = function () {
		...
		var stepindex = 1;
		var i;
		for (i = 0; i < nexacro.UXFORM_CANVAS_SIZE_X; i += dist) {
		...
		for (i = 0; i < nexacro.UXFORM_CANVAS_SIZE_Y; i += dist) {
			snap_manager.add_info(nexacro.SnapInfo.INFO_VERT, new nexacro.SnapInfo(i, nexacro.SnapInfo.SNAP_DOT));
		}

		for (i = -dist; i > -pt.y; i -= dist) {
			snap_manager.add_info(nexacro.SnapInfo.INFO_VERT, new nexacro.SnapInfo(i, nexacro.SnapInfo.SNAP_DOT));
		}
	};
	...
	_pSnapManager.move_guideline = function (pos) {
		...
		for (var i = 0; i < count; i++) {
			...
			if (!info.resize) {
				var side = 0;
				nexacro._executeEvalStr("side = info.rc." + info.side + " + (pos - info.rc." + info.target + ")");
				nexacro._executeEvalStr("info.rc." + info.side + " = " + side);
			}

			nexacro._executeEvalStr("info.rc." + info.target + " = pos;");
		}
	};
	...
	_pTrackerLayer.check_cursor = function (x, y) {
		...
		if (nexacro._design_mode == nexacro._design_mode_form_move) {
			...
			if (designform._selected_components.length > 0) {
				...
				for (var i = 0; i < count; i++) {
					var obj = comps[i];
					rect = designform._hitTestTracker(x, y, rootobj, obj);
				...
				rect = designform._getClientRect(rootobj);
	...
	_pTrackerLayer._drag_move = function (x, y) {
		...
		for (i = 0; i < count; i++) {
			...
			rect = designform._getComponentRect(comp, true);
			rect[0] += x;
			rect[1] += y;
	...
	_pTrackerLayer.execLButtonUp = function (x, y) {
		...
		if (this.target.get_cursor_type() == nexacro._mouse_cursor_create_component) {
	...
	_pTrackerLayer.execMouseMove = function (x, y) {
		var designform = this.target;
		var move_info, realform;
		if (this._is_dragging()) {
			move_info = designform._move_info;
			...
			if (bApply) {
				move_info = this.target._move_info;
				var rootobj = designform.get_root_obj();
				var rootid = designform._getScopeName(rootobj);
				realform = false;
				var parentid, rect, comps, comp, count, i, l, t;
					...
					case nexacro._mouse_cursor_move:
						{
							...
							comps = designform._selected_components;
							count = move_comps.length;
							for (i = 0; i < count; i++) {
								comp = comps.get_item(i);
								rect = move_comps[i].split(',');

								parentid = designform._getScopeName(comp.parent);
								l = designform.rootToParent(rect[0], true, parentid);
								t = designform.rootToParent(rect[1], false, parentid);
					...
					case nexacro._mouse_cursor_create_component:
						{
							...
							parentid = designform.checkParent(move_info.x1, move_info.y1, rootid);

							var parentobj = designform._getObject(parentid);
							while (parentobj && parentobj != designform._inner_form) {
								rect = designform._getClientRect(parentobj);
					...
					default:
						{
							...
							while (comp) {
								...
								if (comp == designform._inner_form) {
									...
									parentid = designform._getScopeName(comp.parent);
									l = designform.rootToParent(parseInt(rect[0]), true, parentid);
									t = designform.rootToParent(parseInt(rect[1]), false, parentid);
			...
			case nexacro.Event.KEY_TAB:
				{
					...
					pivot = designform._selected_components.get_pivot();
					if (pivot) {
						parent = pivot.parent;
	...
	_pTrackerLayer.execDrawBack = function (hDC) {
		...
		var obj, rect;
		if (count == 0) {
			obj = designform.get_root_obj();
			rect = designform._getClientRect(obj);
			...
			for (var i = 0; i < count; i++) {
				...
				function _is_use_position (pos) {
					return nexacro._executeEvalStr("!(undefined === obj." + pos + " || null === obj." + pos + ")");
				}
				...
				case nexacro._mouse_cursor_move:
					{
						...
						draw_info = this._make_draw_info();
						designform.drawMoveRect(hDC, draw_info);

						if (designform.is_need_snap(cursor_type)) {
							draw_info = this._make_snap_draw_info();
	...
	_pInfoLayer.execDrawBack = function (hDC) {
		...
		if (designform._show_taborder_info) {
			info = this._make_taborder_info(designform._inner_form);
	...
	_pRulerLayer.check_cursor = function (x, y) {
		...
		switch (type) {
			case this._ruler_h:
				...
				index = this._exsit_guideline(pos, true);
				if (index >= 0) {
			...
			case this._ruler_v:
				...
				index = this._exsit_guideline(pos, false);
				if (index >= 0) {
	...
	_pRulerLayer.execLButtonDblClk = function (x, y) {
		...
		switch (type) {
			case this._ruler_button:
				break;
			case this._ruler_h:
				pos = this.target.clientToForm(x, true);
				this.remove_guideline(pos, true);
				this.target.drawWindow(false);
				break;
			case this._ruler_v:
				pos = this.target.clientToForm(y, false);
	...
	_pDesignForm.onMouseMove = function (x, y) {
		...
		if (this._dragging_layer >= 0) {
			layer = this._layer_list[this._dragging_layer];
