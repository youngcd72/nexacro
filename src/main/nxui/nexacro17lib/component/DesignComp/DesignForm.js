...
if (!nexacro.DesignForm) {
	...
	nexacro._getComputedStyleProperties = function (control_elem, propid, ret_handle, convert_prop) {
		...
		propids.forEach(function (val, idx, arr) {
			switch (val) {
				case "wordSpacing":
					arr[idx] = "word-spacing";
					break;
				case "letterSpacing":
					arr[idx] = "letter-spacing";
					break;
				case "textDecoration":
					arr[idx] = "-nexa-text-decoration";
					break;
				case "wordWrap":
					arr[idx] = "-nexa-word-wrap";
					break;
				case "borderRadius":
					arr[idx] = "border-radius";
					break;
				case "border":
					arr[idx] = "-nexa-border";
					break;
				...
				case "boxShadow":
					...
					break;
				case "padding":
					arr[idx] = "-nexa-padding";
					break;
				case "textAlign":
					arr[idx] = "-nexa-text-align";
					break;
				case "verticalAlign":
					arr[idx] = "-nexa-vertical-align";
					break;
				case "icon":
					arr[idx] = "-nexa-icon";
					break;
				case "iconPosition":
					arr[idx] = "-nexa-icon-position";
					break;
			}
		});
		...
		if (convert_prop) {
			...
			for (var stylename in tmp) {
				switch (stylename) {
					case "word-spacing":
						tmp.wordSpacing = tmp[stylename];
						delete tmp[stylename];
						break;
					case "letter-spacing":
						tmp.letterSpacing = tmp[stylename];
						delete tmp[stylename];
						break;
					case "-nexa-text-decoration":
						tmp.textDecoration = tmp[stylename];
						delete tmp[stylename];
						break;
					case "-nexa-word-wrap":
						tmp.wordWrap = tmp[stylename];
						delete tmp[stylename];
						break;
					case "border-radius":
						tmp.borderRadius = tmp[stylename];
						delete tmp[stylename];
						break;
					case "-nexa-border":
						tmp.border = tmp[stylename];
						delete tmp[stylename];
						break;
					case "-nexa-edge":
						tmp.edge = tmp[stylename];
						delete tmp[stylename];
						break;
					case "box-shadow":
						tmp.boxShadow = tmp[stylename];
						delete tmp[stylename];
						break;
					case "-nexa-text-align":
						tmp.textAlign = tmp[stylename];
						delete tmp[stylename];
						break;
					case "-nexa-vertical-align":
						tmp.verticalAlign = tmp[stylename];
						delete tmp[stylename];
						break;
					case "-nexa-icon":
						tmp.icon = tmp[stylename];
						delete tmp[stylename];
						break;
					case "-nexa-icon-position":
						tmp.iconPosition = tmp[stylename];
						delete tmp[stylename];
						break;
					case "-nexa-padding":
	...
	_pDesignForm._showSubLayout = function (obj, bShow, positionstep) {
		...
		if (bShow) {
			...
			obj._adjustPosition = function () {
				...
				var calc_pos;
				var calc_left, calc_top, calc_right, calc_bottom;
	...
	_pDesignForm.refreshLayout = function (compid) {
		...
		if (pManager) {
			...
			if (new_idx > -1) {
				var new_layout = obj._layout_list[new_idx];
				if (new_layout) {
	...
	_pDesignForm.getViewTemplateResult = function (filepath, src_script, fieldarray, contents, generationattr) {
		var ret;
		try {
			(new Function(src_script))();

			if (typeof fn_GetViewGenerationResult === "function") {
	...
	_pApplicationAccessPort.insertConstColumn = function (datasetid, index, columnid, type, size, value, datapath) {
		...
		if (ds && ds instanceof nexacro.Dataset) {
			...
			if (ds._constVars.insert_item(index, columnid, new nexacro.ConstColumnVariable(columnid, value, type, ntype, size, datapath, index)) >= 0) {
				...
				if (ds._eventstat) {
					ds.on_fire_onrowsetchanged(-1, -1, 34);
				}
			}
		}
	};
	...
	_pFormAccessPort.getViewTemplateResult = function (filepath, src_script, fieldarray, contents, generationattr) {
		return this.target.getViewTemplateResult(filepath, src_script, fieldarray, contents, generationattr);
	};
