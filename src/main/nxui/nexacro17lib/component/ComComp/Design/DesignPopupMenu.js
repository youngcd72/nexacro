...
if (nexacro.PopupMenu) {
			...
			if (ds && this.levelcolumn && this.captioncolumn && this.idcolumn) {
				...
				for (var rowlength = datarow; rowlength < len; rowlength++) {
					...
					if (level == this.level) {
						...
						var hotkeytext = ds.getColumn(rowlength, this.hotkeycolumn);
						if (hotkeytext) {
							popupmenuitem._setHotkeyText(hotkeytext);
						}
	...
	_pPopupMenu._reCalcSize = function () {
		...
		if (ds && this.captioncolumn) {
			...
			var item_width;
			if (this.level == 0) {
				item_width = itempadding_l + itempadding_r + (textcontrol_width ? textcontrol_width + gap : 0) + (has_expand ? expimgwidth ? expimgwidth + gap : gap + expandtext_width : 0);
			}
			else {
				item_width = itempadding_l + itempadding_r + icontextpadding + (iconwidth ? iconwidth : 0) + (textcontrol_width ? textcontrol_width + gap : 0) + (hotkeycontrol_width ? hotkeycontrol_width + gap : 0) + (has_expand ? expimgwidth ? expimgwidth + gap : gap + expandtext_width : 0);
			}
