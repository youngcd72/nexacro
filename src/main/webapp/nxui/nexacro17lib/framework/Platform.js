...
if (!nexacro._bInitPlatform) {
	...
	__pLoadManager.on_load_dataobjectmodule = function (svcid, errstatus, message, fireerrorcode, returncode, requesturi, locationuri, extramsg, dataobj) {
		var load_Item = this.getDataItem(svcid);
		if (load_Item) {
			var callback_id = load_Item._context_callback;
			var callback_func = this.context[callback_id];
			...
			if (errstatus < 0 && fireerrorcode) {
				...
				if (dataobj && (fireerrorcode != "comm_cancel_byuser" || fireerrorcode != "comm_stop_transaction_byesc" || 
					load_Item._is_cancel || !load_Item.handle || (load_Item.handle && !load_Item.handle._user_aborted && load_Item.handle._user_aborted !== undefined))) {
					ret = dataobj._onDataObjectRequestError(dataobj, true, dataobj, fireerrorcode, requesturi, returncode, requesturi, locationuri, extramsg, svcid);
				...
				if (windowX != dragInfo.startX || windowY != dragInfo.startY) {
					...
					if (comp) {
						...
						if (ret && ret[0] === true && nexacro._cur_drag_info) {
							...
							comp._on_dragmove(elem, dragInfo.target, dragInfo.referTarget, dragInfo.data, dragInfo.userdata, dragInfo.datatype, dragInfo.filelist, strButton, altKey, ctrlKey, shiftKey, canvasX, canvasY, screenX, screenY, undefined, undefined, undefined, xdeltavalue, ydeltavalue);
							var c = comp;
							var is_scroll = false;
				...
				if (!dragInfo.isDragging) {
					if (windowX != dragInfo.startX || windowY != dragInfo.startY) {
						...
						if (retDrag && retDrag[0] === true) {
							...
							comp._on_dragmove(drag_elem, dragInfo.target, dragInfo.referTarget, dragInfo.data, dragInfo.userdata, dragInfo.datatype, dragInfo.filelist, "touch", false, false, false, canvasX, canvasY, screenX, screenY, undefined, undefined, undefined, xdeltavalue, ydeltavalue);
					...
					if ((comp && !dragInfo.isSelfAction) || comp && (ischild && dragInfo.isSelfAction)) {
						if (prev_elem == elem) {
							...
							comp._on_dragmove(elem, dragInfo.target, dragInfo.referTarget, dragInfo.data, dragInfo.userdata, dragInfo.datatype, dragInfo.filelist, "touch", false, false, false, canvasX, canvasY, screenX, screenY, undefined, undefined, undefined, xdeltavalue, ydeltavalue);
						}
						else {
							var from_comp;
